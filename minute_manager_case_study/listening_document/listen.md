# listen.md — how to engage with this document

This is a listening document for *minute*. It's structured for sequential engagement, not reference. Read in order.

## context
- **artist**: Gamma
- **year**: 2026
- **genre**: orchestral villain song / musical theater
- **album**: A Study in Time
...
2. **Listen section by section.** Go through `sections/01_*.md` through `sections/09_*.md` **in order, one at a time**. For each one:
...
5. **Only after the sequential pass:** Then re-examine any section you want a second pass on.
